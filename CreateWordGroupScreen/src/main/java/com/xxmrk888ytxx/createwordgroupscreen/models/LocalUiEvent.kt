...
internal sealed class LocalUiEvent : UiEvent {
    ...
    class PickImageRequestEvent(val contract: ActivityResultLauncher<PickVisualMediaRequest>) : LocalUiEvent()

    class ImagePickedEvent(val imageUri: Uri?) : LocalUiEvent()

    class BackPageEvent(val pagerState: PagerState,val uiScope:CoroutineScope,val navigator: Navigator) : LocalUiEvent()
